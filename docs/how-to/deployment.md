...
> Окружения, CI/CD, конфигурация и rollout self-hosted-инсталляции одного предприятия.
...
Application API, Worker и PostgreSQL разворачиваются в локальном контуре предприятия.
Общего SaaS runtime между предприятиями нет. `LineGateway` работает в центре; Edge
Agent не разворачивается до прохождения [edge gate](../planning/architecture-validation.md#gate-edge).
...
- Restore test обязателен до destructive/long-running migration

**Initial schema:** see [data-model.md](../reference/data-model.md)
...
- Alerts: see [integration-plan.md](../explanation/integration-plan.md) §12
...
Backup содержит raw КМ и должен быть зашифрован. Базовые цели: RPO ≤ 24 ч для
метаданных, отсутствие потери подтверждённых событий и RTO ≤ 4 ч; конкретная
инсталляция обязана подтвердить или заменить эти значения.
