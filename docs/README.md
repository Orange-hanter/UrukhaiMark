...
Документация организована по [Diátaxis](https://diataxis.fr/): каждый файл отвечает ровно на один тип вопроса. Этот файл — единственный индекс.

| Раздел | Вопрос читателя | Когда открывать |
|--------|------------------|------------------|
| [`tutorials/`](#tutorials) | «Как мне начать с нуля?» | Разовый онбординг |
| [`how-to/`](#how-to) | «Как мне сделать конкретную задачу?» | Есть контекст, нужен рецепт |
| [`reference/`](#reference) | «Какое точное значение/лимит/поле?» | Нужны факты |
| [`explanation/`](#explanation) | «Почему это устроено именно так?» | Нужен контекст |

Отдельно, **вне** Diátaxis (управление проектом):

| Раздел | Назначение |
|--------|------------|
| [`planning/`](#planning) | Roadmap, validation, WBS, governance, открытые вопросы |
| [`decisions/`](#decisions) | ADR — зафиксированные архитектурные решения |
...
| [obsidian-kb.md](reference/obsidian-kb.md) | Индекс связанных заметок Obsidian (вне репо) |
