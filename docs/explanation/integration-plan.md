...
|---------|------------|----------|--------|------|-------------|
| GS1 Бел | GCP, GLN | Web + договор | Prod | 0 / manual | Блокирующая |
| ePASS | GTIN, описание товара | Web/API | Prod | 0 / manual | Блокирующая |
| datamark.by | КМ, отчёты, отгрузки | HTTPS REST | Sandbox→Prod | MVP | Блокирующая |
| datamark UKZ API | УКЗ для пива | HTTPS REST | Prod | Future / Phase 3 | Средняя |
| Честный знак РФ | Приёмка (контрагент) | Через datamark ships | Prod | MVP / indirect | Блокирующая для RF |
| CRPT True API | Пивo РФ | HTTPS + УКЭП | Prod | Blocked / Phase 4 | Отложена |
| PostgreSQL | Audit, state | TCP | Local/Prod | 1 | Блокирующая |
| Принтер | Этикетки | ZPL/IP или USB | Prod | 1 | Блокирующая |

## 2.1 Граница provider adapters

Core вызывает не универсальный метод «изменить статус», а capability-specific ports:
`OrderCodes`, `SubmitMarkReport`, `SubmitManufactureReport`, `SubmitShipment`.
UKZ и CRPT реализуют отдельные pipeline. Если provider не объявляет capability,
операция недоступна; core не эмулирует юридический процесс.
...
```
API response → DB BYTEA → validated adapter string → Reports API / DataMatrix encoder
```
...
Направление master data, конфликт-стратегия и transport пока не выбраны. Поэтому
ERP — `Future adapter`, а не system of record MVP. ЭДО, marketplace FBO/FBS и
серверное подписание также остаются `Future` до отдельного ADR/контракта.

## 9.1 Line Gateway и Edge (Next/Future)

MVP вызывает printer/scanner через локальный `LineGateway`. Будущий Edge Agent
получает только арендованный набор КМ и отправляет at-least-once события через
outbox. Контракт требует `event_id`, sequence, allocation и code hash. Протокол
REST/WebSocket/MQTT выбирается отдельным ADR после измерений; семантика от него
не зависит.
...
| 2 | datamark prod, contracts/agents, durable print/audit |
| 3 | UKZ API |
| 4 | CRPT (if unblocked) |
| 5+ | Aggregation, line gateway, edge, ERP/ЭДО — только после gates |
...
| Scenario | Action |
|----------|--------|
| Bad batch printed | Do not submit addMark; isolate physical labels and reconcile per confirmed provider procedure |
...
## 13.1 Неподтверждённые контракты

До первичной спецификации или sandbox evidence архитектура не обещает:

- программный вывод из оборота через datamark;
- автоматическую отмену ошибочной BY-отгрузки;
- конкретные сроки аннулирования неиспользованных BY/RF КМ;
- одинаковую модель «ввода в оборот» для datamark и CRPT;
- обязательную схему ЭДО/FBO/FBS для сценариев вне MVP.

Реестр доказательств: [architecture-validation.md](../planning/architecture-validation.md).
